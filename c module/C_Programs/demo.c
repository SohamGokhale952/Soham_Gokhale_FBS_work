...
parr = (Player *)malloc(old_size * sizeof(Player));
...
{
    *parr = (Player *)realloc(*parr, (*old_size + n) * sizeof(Player));
    if (*parr == NULL)
    {
...
void removed(Player **parr, int *old_size)
{
    ...
    for (int i = 0; i < *old_size; i++)
    {
        ...
        {
            found = 1;
            for (int j = i; j < *old_size - 1; j++)
            {
                (*parr)[j] = (*parr)[j + 1];
            }
            (*old_size)--;

            *parr = (Player *)realloc(*parr, (*old_size) * sizeof(Player));
